  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-2xs">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900">Үйлчилгээ</h1>
          <p className="text-xs text-slate-400 mt-0.5">Таны бизнес эрхлэгчийн үзүүлж буй үйлчилгээний жагсаалт</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 rounded-3xl border border-slate-100 text-center space-y-3">
          ...
          <p className="text-sm font-bold text-slate-700">Үйлчилгээ олдсонгүй</p>
          <p className="text-xs text-slate-400">Шинэ үйлчилгээ нэмж бүртгэнэ үү.</p>
        </div>
      ) : viewMode === 'list' ? (
        // TABLE VIEW
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-400 truncate max-w-xs block">{service.description || 'Тайлбар байхгүй'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredServices.map((service) => (
            <div key={service.service_id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-2xs space-y-4 hover:border-blue-200 transition-all">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-sm text-slate-900">{service.name}</h3>
              ...
              <p className="text-xs text-slate-500 line-clamp-2">{service.description || 'Тайлбар байхгүй'}</p>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white max-w-lg w-full rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <h3 className="font-black text-base text-slate-900">Шинэ үйлчилгээ бүртгэх</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Үйлчилгээний нэр *</label>
                <input 
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Ангилал</label>
                <input 
                  type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1">Хугацаа (минутаар)</label>
                  <input 
                    type="number"
                    placeholder="30"
